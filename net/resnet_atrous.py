...
class ResNet_Atrous(nn.Module):
    ...
    def __init__(self, block, layers, atrous=None, output_stride=16):
        ...
        if output_stride == 8:
            ...
            raise ValueError('resnet_atrous.py: output stride=%d is not supported.'%output_stride) 
...
#                      )
        ...
        self.layer3 = self._make_layer(block, 512, 256, layers[2], stride=stride_list[1], atrous=16//output_stride)
        self.layer4 = self._make_layer(block, 1024, 512, layers[3], stride=stride_list[2], atrous=[item*16//output_stride for item in atrous])
        #self.layer5 = self._make_layer(block, 2048, 512, layers[3], stride=1, atrous=[item*16//output_stride for item in atrous])
        #self.layer6 = self._make_layer(block, 2048, 512, layers[3], stride=1, atrous=[item*16//output_stride for item in atrous])
        #self.layer7 = self._make_layer(block, 2048, 512, layers[3], stride=1, atrous=[item*16//output_stride for item in atrous])
        self.layers = []
...
def resnet50_atrous(pretrained=True, output_stride=16, **kwargs):
    """Constructs a atrous ResNet-50 model."""
    model = ResNet_Atrous(Bottleneck, [3, 4, 6, 3], atrous=[1,2,1], output_stride=output_stride, **kwargs)
    if pretrained:
...
def resnet101_atrous(pretrained=True, output_stride=16, **kwargs):
    """Constructs a atrous ResNet-101 model."""
    model = ResNet_Atrous(Bottleneck, [3, 4, 23, 3], atrous=[2,2,2], output_stride=output_stride, **kwargs)
    if pretrained:
...
def resnet152_atrous(pretrained=True, output_stride=16, **kwargs):
    """Constructs a atrous ResNet-152 model."""
    model = ResNet_Atrous(Bottleneck, [3, 8, 36, 3], atrous=[1,2,1], output_stride=output_stride, **kwargs)
    if pretrained:
